...
export type NewReport = {
  ...
  userAgent?: string | null; reporter?: string | null
}
...
export const repo = {
  ...
  createReport(x: NewReport): Report {
    ...
    conn.prepare(
      `INSERT INTO reports (id, project_id, note, screenshot_url, page_url, viewport, user_agent, reporter, status, created_at)
       VALUES (?,?,?,?,?,?,?,?,?,?)`,
    ).run(id, x.projectId, x.note, x.screenshotUrl ?? null, x.pageUrl ?? null, x.viewport ?? null, x.userAgent ?? null, x.reporter ?? null, 'new', Date.now())
